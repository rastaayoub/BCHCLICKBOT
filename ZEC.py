 ...
 async def connection(num):
  ...
  async def boter(msms):
    ...
    # print(botUN,startID)
     ...
     await PROFILER.forward_messages('Zcash_click_bot', mass[0], botUN)
     if mass[0].message.find('/start') >0 :
      await PROFILER.send_message('Zcash_click_bot','a')
      await ayoB()
    else:
  ...
  PROFILER.add_event_handler(EVENTER,events.NewMessage(incoming=True,chats="Zcash_click_bot"))
  ...
  PROFILER.add_event_handler(SWITCH,events.NewMessage(incoming=True,chats="Zcash_click_bot",pattern='Sorry, there are no new ads available.'))
  ...
  PROFILER.add_event_handler(Repeter,events.NewMessage(incoming=True, chats="Zcash_click_bot",pattern='Sorry, that task is no longer valid.'))
  ...
  PROFILER.add_event_handler(SAliveJ,events.NewMessage(incoming=True,chats="Zcash_click_bot",pattern='Success!'))
  ...
  PROFILER.add_event_handler(SAliveV,events.NewMessage(incoming=True,chats="Zcash_click_bot",pattern='You earned'))
  ...
  PROFILER.add_event_handler(SKIP,events.NewMessage(incoming=True,chats="Zcash_click_bot",pattern='We cannot'))
  ...
  async def SKIPB(event):
   if type(event.original_update):
    ...
    await ayoB()
  PROFILER.add_event_handler(SKIPB,events.NewMessage(incoming=True,chats="Zcash_click_bot",pattern='Sorry, that is not a valid forwarded message.'))
